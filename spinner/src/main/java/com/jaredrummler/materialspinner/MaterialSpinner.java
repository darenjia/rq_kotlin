...
public class MaterialSpinner extends TextView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            int defaultTextGravity = 0;
            textGravity = ta.getInteger(R.styleable.MaterialSpinner_ms_text_gravity, defaultTextGravity);
            //popupTextGravity = ta.getDimensionPixelSize(R.styleable.MaterialSpinner_ms_text_gravity, defaultTextGravity);
            getTextGravity(textGravity);
            popupPaddingTop =
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                setText(adapter.getShowText(position));
                collapse();
    ...
    private void getTextGravity(int type) {
        if (type == 0) {
            textGravity = Gravity.CENTER;
        } else if (type == 1) {
            textGravity = Gravity.START | Gravity.CENTER_VERTICAL;
        } else {
            textGravity = Gravity.END | Gravity.CENTER_VERTICAL;
        }
    }
    ...
    public <T> void setAdapter(MaterialSpinnerAdapter<T> adapter) {
        ...
        this.adapter.setPopupPadding(popupPaddingLeft, popupPaddingTop, popupPaddingRight, popupPaddingBottom, textGravity);
        setAdapterInternal(adapter);
    }
    ...
    private void setAdapterInternal(@NonNull MaterialSpinnerBaseAdapter adapter) {
        ...
        if (adapter.getItems().size() > 0) {
            if (nothingSelected && !TextUtils.isEmpty(hintText)) {
                ...
                setTextColor(textColor);
                setText(adapter.getShowText(selectedIndex));
            }
        } else {
            setText("");
        }
