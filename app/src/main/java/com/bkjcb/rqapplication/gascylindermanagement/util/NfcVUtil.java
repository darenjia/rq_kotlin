...
public class NfcVUtil {
     ...
     *  
     */
    public byte[] readOneBlockForByte(int position) throws IOException {
        byte cmd[] = new byte[11];
        cmd[0] = (byte) 0x22;
        cmd[1] = (byte) 0x20;
        System.arraycopy(ID, 0, cmd, 2, ID.length); // UID
    ...
    public List<byte[]> readBlocksByte(int begin, int count) throws IOException {
        List<byte[]> list = new ArrayList<>();
        if ((begin + count) > blockNumber) {
            count = blockNumber - begin;
        }
        for (int i = begin; i < count + begin; i++) {
            list.addAll(Arrays.asList(readOneBlockForByte(i)));
            Log.i("TAG", "readBlocksByte: "+i);
        }
        return list;
    }

    public List<byte[]> readAllBlocksByte() throws IOException {
        List<byte[]> list = new ArrayList<>();
    ...
    private String printHexStrings(byte[] data) {
        StringBuffer s = new StringBuffer();
        for (int i = 0; i < data.length; i++) {
            String hex = Integer.toHexString(data[i] & 0xFF);
            if (hex.length() == 1) {
