...
public class ImageListAdapter extends BaseQuickAdapter<String, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, String item) {
        ...
        if (item.contains("http")) {
            Glide.with(helper.itemView).load(item).apply(GlideUtil.INSTANCE.obtainRequestOption()).into(imageView);
        } else {
            Glide.with(helper.itemView).load(new File(item)).apply(GlideUtil.INSTANCE.obtainRequestOption()).into(imageView);
        }
