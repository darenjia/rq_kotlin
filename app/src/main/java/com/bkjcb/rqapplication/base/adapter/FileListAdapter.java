...
public class FileListAdapter extends BaseQuickAdapter<MediaFile, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, MediaFile item) {
        ...
        if (item.isLocal()) {
            if (item.getType() == 1) {
                Glide.with(helper.itemView).load(item.getPath()).apply(GlideUtil.INSTANCE.obtainRequestOption()).into(imageView);
            } else {
                Glide.with(helper.itemView).load(getFileImage(item)).apply(GlideUtil.INSTANCE.obtainRequestOption()).into(imageView);
            }
        } else {
            Glide.with(helper.itemView).load(item.getPath()).apply(GlideUtil.INSTANCE.obtainRequestOption()).into(imageView);
        }
